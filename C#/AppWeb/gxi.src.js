...
gx.http_i = (function ($) {
		...
		return {
			...
			doCall: function (info) {
				info.gxInitiator = gx.http.currentUrl();
				gx.util.alert.hide();
			...
			doCallHandler: function (req, info) {
				...
				if( info.gxInitiator &&  info.gxInitiator != gx.http.currentUrl())
					return;
				if (info.obj)
...
(function($) {
	...
	var FEEDBACK_MASK_DELAY = gx.NotificationDelay === undefined ? 200 : gx.NotificationDelay;
